...
let Route = (function () {
  ...
  Route.prototype.build = function build($scope, path, options) {
    _import2['default'].defaults(options, $scope.get('options'));

    this.defaultController = options.controller || $scope.get('controller');
    this.defaultAction = options.action || $scope.get('action');

    this.defaults = _import2['default'].defaults(options.defaults || {}, $scope.get('defaults') || {});
    this.$scope = $scope;
  ...
  Route.prototype.normalizeOptions = function normalizeOptions(options, formatted, pathParams, pathAst, modyoule) {
    if (pathParams.filter(function (p) {
      ...
      if (!_import2['default'].has(options, 'controller')) {
        options.controller = /.+?/;
      }
    }

    let toEndpoint = _utils2['default'].splitTo(this.to);
    ...
    options = _import2['default'].assign(options, this.checkControllerAndAction(pathParams, this._controller, this._action));
    return options;
  };
  ...
  Route.prototype.checkControllerAndAction = function checkControllerAndAction(pathParams, controller, action) {
    let hash = this.checkPart('controller', controller, pathParams, {}, function (part) {
      if (_import2['default'].isRegExp(part)) return part;
  ...
  Route.prototype.addControllerModule = function addControllerModule(controller, modyoule) {
    if (modyoule && !_import2['default'].isRegExp(controller)) {
      if (/^\//.test(controller)) {
        ...
        return _import2['default'].compact([modyoule, controller]).join('/');
      }
    }
    return controller;
  };
  ...
  _createClass(Route, [{
    ...
    key: 'verb',
    get: function () {
      return _import2['default'].isArray(this.options.verb) ? this.options.verb : [this.options.verb];
    }
  }]);
...
// Delegates
_delegate2['default'](Route.prototype, 'options').getter('format').getter('as').getter('to');
